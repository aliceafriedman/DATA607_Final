...
![](https://github.com/aliceafriedman/Data607_Final/Bikelash.png)
...
###Step 4: Run the function to download the file

The following code will check to see if you have a "data/temp" folder, and then create one if it does not exist, and then download the file(s) to that folder. This will appear in your working drive, wherever that is set.
...
###Step 5: Unzip the file to access and load the CSV into a data frame
...
###Step 6: Read shapefile using ```rgdal```
...
JSON data from an API is a lot easier to load (2 steps instead of 7) than parsing a website to find a zipped file containing a shapefile!
...
#note: I have stored my private Google API key
...
ggmap(get_map("New York", maptype = "terrain-background", zoom = 11)) +
  geom_line(data = bikesYearGoogle,                      #add bike map
            aes(x = long, y = lat, group = group),
            alpha = .25) +           
  geom_line(data = filter(bikesYearGoogle, year > 2012), #add highlight for lines 2012-2017
            aes(x = long, y = lat, group = group, 
